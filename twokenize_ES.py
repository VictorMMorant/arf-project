...
aa1 = r'''([A-Za-zÁÉÍÓÚáéíóúüÜÑñ]\.){2,}''' + pos_lookahead(BoundaryNotDot)
aa2 = r'''([A-Za-zÁÉÍÓÚáéíóúüÜÑñ]\.){1,}[A-Za-zÁÉÍÓÚáéíóúüÜÑñ]''' + pos_lookahead(BoundaryNotDot)
...
def apply_stemming(tokens) :
  return [ps.stem(word).lower() for word in tokens]
...
NotEdgePunct = r"""[A-Za-zÁÉÍÓÚáéíóúüÜ0-9Ññ]"""
...
#    print u" ".join(tokenize(line[:-1])).encode('utf-8')
 #   text = u" ".join(tokenize(line[:-1])).encode('utf-8') /*LA BONA*/
  #  line = line.decode("utf8")
    ...
    text3=emoticons_ES.analyze_tweet(text, http=True, emo=True, arroba=False, almo=True, web=True)
